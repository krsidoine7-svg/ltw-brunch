# Intégration Chefskrsidoine7 — skill-mermaidH

Ce skill s’exécute sous l’orchestration de **Chefskrsidoine7** (manager du projet). Il sert à **visualiser les flux** avant, pendant et après les décisions — pas seulement pour documenter a posteriori.

**Chemins relatifs** (depuis `Chefskrsidoine7/`) :
- Manager : `./SKILL.md`
...
```mermaid
flowchart TD
    User([Utilisateur]) --> Chef[Chefskrsidoine7]
...
## Quand diagrammer (déclencheurs Chefskrsidoine7)
...
**Règle** : si la demande touche l'un de ces contextes, Chefskrsidoine7 **propose ou impose** un diagramme avant validation du plan d'exécution.
...
- Un sous-agent **ne charge pas** `skill-mermaidH` pour un autre agent sans `[DÉLÉGATION APPROUVÉE]` de Chefskrsidoine7.
...
| Phase skill-mermaidH | Action Chefskrsidoine7 |
|----------------------|------------------------|
...
sequenceDiagram
    ...
    participant Chef as Chefskrsidoine7
