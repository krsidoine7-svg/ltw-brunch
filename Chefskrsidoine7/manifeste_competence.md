# Manifeste de Compétences Principal - Chefskrsidoine7

Ce manifeste répertorie l'ensemble des sous-agents et banques de compétences gérés par **Chefskrsidoine7**. 
...
- **Rôle** : Analyse de code, recherche de motifs, et tests de performance.
- **Utilisation** : À solliciter pour auditer la base de code, trouver des structures récurrentes, comprendre l'architecture existante, ou exécuter/analyser des tests de charge.
...
- **🧪 Skill attaché : `run-load-test`**
  - **Lien** : [📁 Consulter le SKILL](./banques_skills/skills-globales/run-load-test/SKILL.md)
  - **Rôle** : Expert en tests de charge, de stress, de pic et d'endurance avec Locust. Analyse des résultats, détection des points de rupture, et rapports.
  - **Déclencheurs** : Tests de charge, stress testing, performances applicatives, mesure de temps de réponse, identification de point de rupture, validation sous contrainte de charge.
...
- **🛡️ Skill attaché : `skill-mermaidH`**
  ...
  - **Rôle** : Skill unifié Mermaid (flowchart, séquence, ERD, C4, UX, onboarding, workflows). Délégation orchestrée par Chefskrsidoine7.
...
En plus des sous-agents, Chefskrsidoine7 dispose de banques de compétences intégrées dans le dossier `banques_skills/` :
...
### `memoire-favor`
- **Lien** : [📁 Accéder à la Mémoire](./banques_skills/memoire-favor/)
...
Lorsqu'un agent souhaite déléguer à un autre agent listé ci-dessus, ou utiliser un outil des banques de compétences, il doit **obligatoirement** obtenir la validation préalable de Chefskrsidoine7.
